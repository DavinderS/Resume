...
Player.prototype.onHit = function(damage) {
    // might get hit by multiple bullets at the same time, so need to check if it"s greater than 0 or we get an error
    if (this.health > 0) {
        this.health -= damage;
        this.healthBar.attr({
...
Player.prototype.updateLocation = function(keys, shake, background) {
    ...
    if (this.x > window.innerWidth / 2 && this.x < background.width - window.innerWidth / 2)
    {
        this.screenOffsetX = this.x - window.innerWidth / 2;
    }
    if (this.y > window.innerHeight / 2 && this.y < background.height - window.innerHeight / 2) 
    {
        this.screenOffsetY = this.y - window.innerHeight / 2;
    }
